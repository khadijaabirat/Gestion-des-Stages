...
export default function ProMentorship() {
  ...
  const features = [
    { icon: 'psychology', title: 'Bilan de compétences', desc: 'Profilage professionnel approfondi', color: 'primary' as const, delay: '0s' },
    ...
    { icon: 'forum', title: 'Interaction en temps réel', desc: 'Accompagnement proactif 24/7', color: 'tertiary' as const, delay: '1s' }
  ];

  return (
    <section ref={sectionRef} className="max-w-7xl mx-auto px-6 lg:px-12 mb-16">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center relative z-10">
          <div className="relative min-h-[400px] rounded-2xl glass-panel border border-white/80 shadow-xl flex items-center justify-center p-8">
            ...
            <div className="relative z-10 flex flex-col gap-6 w-full">
              {features.map((feature, i) => (
                <div
                  key={i}
                  className="glass-panel backdrop-blur rounded-xl p-4 flex items-center gap-4 shadow-sm animate-float"
                  style={{
          ...
          <div className="flex flex-col gap-6 stagger-children">
            <h2 className="font-heading text-3xl md:text-4xl text-on-background font-extrabold">
              Un mentorat <span className="text-transparent bg-clip-text bg-gradient-to-r from-secondary to-tertiary">guidé et structuré</span>
            </h2>
            <p className="text-base text-on-surface-variant">
              Un accompagnement personnalisé. Nous vous aidons à identifier vos points forts pour réussir vos entretiens et décrocher le meilleur stage.
            </p>
          </div>
        </div>
      </div>
