...
import { Search, MapPin, Clock, Bookmark, ChevronDown, Loader2, AlertCircle, ArrowRight, Building2, X } from 'lucide-react';
              ...
              {hasMore && (
                <div className="flex justify-center mt-8">
                  <button
                    ...
                    className="bg-surface-container-high text-on-surface font-semibold px-8 py-3 rounded-full hover:bg-white/80 transition-all flex items-center gap-2 shadow-md disabled:opacity-60"
                  >
                    {loadingMore ? (
                      <><Loader2 className="w-5 h-5 animate-spin" /> Chargement...</>
                    ) : (
                      <>Charger plus d&apos;offres <ChevronDown className="w-5 h-5" /></>
                    )}
                  </button>
                </div>
              )}
