...
export default function OfferCard({ offer, index, isSaved, onToggleSave }: OfferCardProps) {
  return (
    <motion.article
      ...
      className={`glass-panel rounded-2xl p-5 flex flex-col md:flex-row gap-6 cursor-pointer relative overflow-hidden shadow-xl hover:shadow-2xl transition-shadow duration-500 ${
        offer.featured ? 'border-l-4 border-l-primary' : ''
      }`}
    >
      {/* Shimmer Effect */}
      <motion.div
      ...
      >
        <div
          ...
          style={{ backgroundImage: `url(${offer.logo})` }}
        />
      </motion.div>
      ...
      <div className="flex-1 flex flex-col justify-center">
        ...
        <p className="text-primary font-semibold mb-3">{offer.company}</p>
