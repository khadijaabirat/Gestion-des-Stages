...
class CandidatureController extends Controller
{
    ...
    public function index()
    {
        ...
        if ($user->role === 'etudiant') {
            $candidatures = Candidature::with(['offreStage.entreprise:id,nom,photo', 'cv'])
                ->where('user_id', $user->id)
                ->orderBy('created_at', 'desc')
                ->paginate(10);
                
        } elseif ($user->role === 'entreprise') {
            $candidatures = Candidature::with(['etudiant:id,nom,email,telephone,photo,filiere,niveau_etude,bio', 'cv', 'offreStage:id,titre'])
...
if (!$offre->entreprise || $offre->statut !== \App\Enums\OffreStatus::PUBLISHED || $offre->entreprise->is_blocked || !$offre->entreprise->est_valide ) {
...
public function getByOffre($offreId)
{
    ...
    $candidatures = Candidature::with('etudiant:id,nom,email,telephone,filiere,photo,niveau_etude,bio', 'cv')
